...
class FreqaiHybridStrategySpot_simplified(IStrategy):
    ...
    def populate_any_indicators(
        ...
        for t in self.freqai_info["feature_parameters"]["indicator_periods_candles"]:
            ...
            informative[f"{coin}base_sma_{t}"] = ta.SMA(informative[f"{coin}base_{t}"], timeperiod = t)
            ...
            informative[f"%-{coin}-long_signal_{t}"] = np.where(informative[f"%-{coin}-supertrend_{t}"] > informative['close'], 1, 0)
            informative[f"%-{coin}-short_signal_{t}"] = np.where(informative[f"%-{coin}-supertrend_{t}"] < informative['close'], -1, 0)
            ...
            informative[f"%-{coin}-HMA_{t}"]=informative[f"{coin}-wma_sqrt_period_{t}"] - informative[f"{coin}-wma_half_period_{t}"]
        ...
        if set_generalized_indicators:
            ...
            df['&-s-up_or_down'] = np.where((df["close"].shift(-3) <= df["close"]), 'down', df['&-s-up_or_down'])
