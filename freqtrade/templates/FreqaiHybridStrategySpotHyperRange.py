...
class FreqaiHybridStrategySpot(IStrategy):
    ...
    buy_laguerre_min = DecimalParameter(low=0.10, high=0.80, default=0.20, decimals=2, space='buy', optimize=True, load=True)
    buy_laguerre_max = DecimalParameter(low=0.10, high=0.80, default=0.50, decimals=2, space='buy', optimize=True, load=True)
    buy_stochrsi_fastk_min = DecimalParameter(low=0, high=25, default=10, decimals=0, space='buy', optimize=True, load=True)
    buy_stochrsi_fastk_max = DecimalParameter(low=25, high=60, default=50, decimals=0, space='buy', optimize=True, load=True)
    buy_bb_middle_band_slope = DecimalParameter(low=-60, high=90, default=-45, decimals=0, space='buy', optimize=True, load=True)
    buy_bb_lower_band_scope_entry = DecimalParameter(low=0.10, high=0.80, default=0.40, decimals=2, space='buy', optimize=True, load=True)
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            (   
                ...
                (df['bb_middle_band_slope'] > self.buy_bb_middle_band_slope.value)&
                (df['macd_slope']>0)&
                (
                    (
